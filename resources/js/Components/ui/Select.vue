<template>
    <div class="relative" ref="selectRef">
        ...
        <button type="button" @click="isOpen = !isOpen" :class="['w-full flex justify-between items-center px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 cursor-pointer',
            ...
            <div class="flex items-center space-x-2">
                ...
                <X v-if="selectedLabel" @click.stop="clearSelection" type="button"
                    class="w-6 h-6 text-gray-500 p-1 rounded-full hover:bg-gray-200 border-1 border-gray-200 transition-colors" />
                <ChevronDown v-if="!selectedLabel"
                    :class="['w-6 h-6 text-gray-500 transform duration-200 p-1 rounded-full hover:bg-gray-200 border-1 border-gray-200 transition-colors', { 'rotate-180': isOpen }]" />
            </div>
        </button>
